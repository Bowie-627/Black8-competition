...
const UPDATED_DEFAULT_RULES = '赛制：8 位参赛者随机分为 A、B 两组，各 4 人循环比赛；每组前两名交叉晋级半决赛。\n小组赛：单局定胜负。\n淘汰赛：半决赛与决赛均为三局两胜。\n单局比分：胜方记 8 分；负方按当局已进球数记 0–7 分。\n积分排名：优先比较比赛胜场，其次比较累计净胜分。\n文明比赛：不得言语侮辱对手或裁判，不得使用心理战术。\n冠军奖品：获得其他所有人点赞快乐平安名片“台球大王”。\n台球费用：由当天参赛的球员 AA。\n比赛解释权归主办方（缪馨）所有。';
...
const rulesText = () => [PREVIOUS_DEFAULT_RULES, PREVIOUS_ORDERED_DEFAULT_RULES].includes(state.config.rulesText) ? UPDATED_DEFAULT_RULES : (state.config.rulesText || '');
const matchSlotKey = (match) => match.phase === 'group' ? (match.group === 'A' ? 'group-a' : 'group-b') : (match.roundKey || (match.round?.includes('半决赛') ? 'knockout-semifinal' : 'knockout-final'));
const slotForMatch = (match) => {
  const slots = state.config.scheduleSlots || {};
  const key = matchSlotKey(match);
  if (key === 'group-a') return slots.groupA;
  if (key === 'group-b') return slots.groupB;
  const knockout = slots.knockout || {};
  if (key === 'knockout-semifinal') return { date: knockout.date, start: knockout.semifinalStart, end: knockout.semifinalEnd };
  return { date: knockout.date, start: knockout.finalStart, end: knockout.finalEnd };
};
const slotLabel = (match) => { const slot = slotForMatch(match); if (!slot?.date) return '时间待定'; return `${fmtEventDate(slot.date)} ${slot.start || '--:--'}–${slot.end || '--:--'}`; };
const tournamentDateRange = () => {
  const slots = state.config.scheduleSlots || {};
  const dates = [slots.groupA?.date, slots.groupB?.date, slots.knockout?.date].filter(Boolean).sort();
  if (!dates.length) return '待定';
  return dates.length === 1 ? fmtEventDate(dates[0]) : `${fmtEventDate(dates[0])} 至 ${fmtEventDate(dates[dates.length - 1])}`;
};
...
function renderHome() {
  ...
  return `<section class="hero"><div><h1>${esc(state.config.title)}</h1></div><aside class="status-card"><div class="stage-label">赛事阶段</div><div class="stage">${stageLabel(state.config.stage)}</div><div class="meta"><div><span>地点</span>${esc(state.config.location)}</div><div><span>赛事时间</span>${esc(tournamentDateRange())}</div><div><span>报名</span><b class="mini-count">${state.players.length}/8</b> 人</div></div></aside></section>
...
function renderSchedule() { const group = state.matches.filter((m) => m.phase === 'group').sort((a, b) => a.group.localeCompare(b.group) || a.round.localeCompare(b.round)); const knock = state.matches.filter((m) => m.phase === 'knockout'); const list = scheduleTab === 'group' ? group : knock; return `<section class="section"><div class="section-head"><div><h1>赛程与比分</h1></div></div><div class="tabs"><button class="${scheduleTab === 'group' ? 'active' : ''}" data-schedule="group">小组赛 (${group.length})</button><button class="${scheduleTab === 'knockout' ? 'active' : ''}" data-schedule="knockout">淘汰赛 (${knock.length})</button></div>${renderMatchList(list)}</section>${scheduleTab === 'knockout' ? renderBracket() : ''}`; }
...
function adminOverview() {
  const groupGenerated = state.matches.some((m) => m.phase === 'group');
  const canKnock = state.matches.filter((m) => m.phase === 'group').length === 12 && state.matches.filter((m) => m.phase === 'group').every((m) => m.status === 'completed');
  const knockGenerated = state.matches.some((m) => m.phase === 'knockout');
  return `<h1>赛事控制</h1><p>按赛事进程执行。系统会保留每次管理操作的日志。</p><div class="grid two">
    <div class="card"><h2>1. 随机分组</h2><p class="section-note">目前 ${state.players.length}/8 人。随机后会关闭报名。</p><div class="admin-actions"><button class="btn primary" data-action="randomize" ${state.players.length !== 8 || groupGenerated ? 'disabled' : ''}>随机分为 A/B 两组</button></div>${state.groups.A.length ? `<div class="people">${state.groups.A.map((id) => `<span class="person-pill">A · ${esc(id)}</span>`).join('')}${state.groups.B.map((id) => `<span class="person-pill">B · ${esc(id)}</span>`).join('')}</div>` : ''}</div>
    <div class="card"><h2>2. 生成小组赛程</h2><p class="section-note">每组 6 场，共 12 场；小组赛为单局定胜负。</p><button class="btn primary" data-action="generateGroups" ${!state.groups.A.length || groupGenerated ? 'disabled' : ''}>生成循环赛程</button></div>
    <div class="card"><h2>应用三日赛制</h2><p class="section-note">A 组第 1 天、B 组第 2 天，半决赛与决赛第 3 天。会清空全部小组赛比分，但保留选手、分组、对阵、淘汰赛结构和操作日志。</p><button class="btn warn" data-action="applyThreeDayFormat" ${!groupGenerated ? 'disabled' : ''}>清空小组比分并应用</button></div>
    <div class="card"><h2>3. 确认晋级并生成淘汰赛</h2><p class="section-note">A1 vs B2、B1 vs A2；半决赛与决赛均为三局两胜。</p><button class="btn primary" data-action="generateKnockout" ${!canKnock || knockGenerated ? 'disabled' : ''}>生成淘汰赛对阵</button></div>
    <div class="card"><h2>当前阶段</h2><p class="stage">${stageLabel(state.config.stage)}</p><p class="section-note">完成决赛后，系统会自动加冕冠军。</p></div>
  </div>`;
}
...
function adminSettings() { return `<h1>赛事设置</h1><p>首页赛事时间会根据三天赛程自动汇总，无需在此单独填写。</p><form id="settingsForm" class="card"><div class="field"><label>赛事名称</label><input class="input" name="title" value="${esc(state.config.title)}"></div><div class="field"><label>地点</label><input class="input" name="location" value="${esc(state.config.location)}"></div><div class="field"><label>新管理 PIN（留空不改）</label><input class="input" name="newPin" inputmode="numeric" pattern="[0-9]{4}" maxlength="4" placeholder="4 位数字"></div><button class="btn primary">保存设置</button></form>`; }
function adminSchedule() {
  const slots = state.config.scheduleSlots || {};
  const groupA = slots.groupA || {}, groupB = slots.groupB || {}, knockout = slots.knockout || {};
  return `<h1>赛程时间</h1><p>A 组集中在第 1 天，B 组集中在第 2 天；半决赛和决赛同在第 3 天。保存后会批量应用到对应场次。</p><form id="scheduleSlotsForm" class="grid two">
    <div class="card"><h2>A 组比赛日 · 第 1 天</h2><div class="grid three"><div class="field"><label>日期</label><input class="input" type="date" name="groupA_date" value="${esc(groupA.date || '')}" required></div><div class="field"><label>开始时间</label><input class="input" type="time" name="groupA_start" value="${esc(groupA.start || '')}" required></div><div class="field"><label>结束时间</label><input class="input" type="time" name="groupA_end" value="${esc(groupA.end || '')}" required></div></div></div>
    <div class="card"><h2>B 组比赛日 · 第 2 天</h2><div class="grid three"><div class="field"><label>日期</label><input class="input" type="date" name="groupB_date" value="${esc(groupB.date || '')}" required></div><div class="field"><label>开始时间</label><input class="input" type="time" name="groupB_start" value="${esc(groupB.start || '')}" required></div><div class="field"><label>结束时间</label><input class="input" type="time" name="groupB_end" value="${esc(groupB.end || '')}" required></div></div></div>
    <div class="card" style="grid-column:1/-1"><h2>淘汰赛比赛日 · 第 3 天</h2><div class="field"><label>统一日期</label><input class="input" type="date" name="knockout_date" value="${esc(knockout.date || '')}" required></div><div class="grid two"><div><h3>半决赛</h3><div class="grid two"><div class="field"><label>开始时间</label><input class="input" type="time" name="semifinal_start" value="${esc(knockout.semifinalStart || '')}" required></div><div class="field"><label>结束时间</label><input class="input" type="time" name="semifinal_end" value="${esc(knockout.semifinalEnd || '')}" required></div></div></div><div><h3>决赛</h3><div class="grid two"><div class="field"><label>开始时间</label><input class="input" type="time" name="final_start" value="${esc(knockout.finalStart || '')}" required></div><div class="field"><label>结束时间</label><input class="input" type="time" name="final_end" value="${esc(knockout.finalEnd || '')}" required></div></div></div></div></div>
    <button class="btn primary">批量保存三日赛程</button>
  </form>`;
}
function adminRules() { return `<h1>比赛规则</h1><p>每行会在公开规则页中显示为一条规则。</p><form id="rulesForm" class="card"><div class="field"><label>规则内容</label><textarea class="input" name="rulesText" rows="10" maxlength="4000" required>${esc(rulesText())}</textarea></div><button class="btn primary">保存比赛规则</button></form>`; }
...
document.addEventListener('click', async (e) => {
  ...
  if (target.dataset.editPlayer) { const id = target.dataset.editPlayer; const form = $('#adminPlayerForm'); form.elements.id.value = id; form.dataset.originalId = id; form.querySelector('button').textContent = `更新 ${id}`; form.scrollIntoView({ behavior: 'smooth' }); return; }
  ...
  if (target.dataset.action) {
    ...
    const labels = { randomize: '随机分组', generateGroups: '生成小组赛程', generateKnockout: '生成淘汰赛对阵', applyThreeDayFormat: '应用三日赛制' };
    const message = currentAction === 'applyThreeDayFormat'
      ? '确认应用三日赛制？这会清空全部小组赛比分；不会删除选手、分组、对阵、淘汰赛结构或操作日志。'
      : `确认${labels[currentAction]}？`;
    if (!confirm(message)) return;
    try { await action(currentAction); } catch (err) { toast(err.message, true); } return;
  }
...
document.addEventListener('submit', async (e) => { e.preventDefault(); const form = e.target; const formId = form.getAttribute('id'); try {
  if (formId === 'signupForm') { const file = form.elements.avatar.files[0]; const avatarData = file ? await fileToData(file) : ''; state = await api('/api/register', { id: form.elements.id.value, avatar: avatarData }); form.reset(); render(); toast('报名成功，欢迎上场！'); }
  if (formId === 'loginForm') { const pin = form.elements.pin.value; const result = await api('/api/admin/login', { pin }); if (!result.ok) throw Error('PIN 不正确'); activePin = pin; sessionStorage.setItem('black8-pin', pin); closeModal(); view = 'admin'; render(); toast('已进入管理后台'); }
  if (formId === 'adminPlayerForm') { const file = form.elements.avatar.files[0]; await action('playerUpsert', { id: form.elements.id.value, originalId: form.dataset.originalId || '', avatar: file ? await fileToData(file) : '' }); form.reset(); delete form.dataset.originalId; }
  if (formId === 'settingsForm') { await action('settings', Object.fromEntries(new FormData(form))); if (form.elements.newPin.value) { activePin = form.elements.newPin.value; sessionStorage.setItem('black8-pin', activePin); } }
  if (formId === 'scheduleSlotsForm') { const data = new FormData(form); const slots = { groupA: { date: data.get('groupA_date'), start: data.get('groupA_start'), end: data.get('groupA_end') }, groupB: { date: data.get('groupB_date'), start: data.get('groupB_start'), end: data.get('groupB_end') }, knockout: { date: data.get('knockout_date'), semifinalStart: data.get('semifinal_start'), semifinalEnd: data.get('semifinal_end'), finalStart: data.get('final_start'), finalEnd: data.get('final_end') } }; await action('scheduleSlots', { slots }); }
  if (formId === 'rulesForm') { await action('rules', { rulesText: form.elements.rulesText.value }); }
  if (formId === 'scoreForm') { if (!form.elements.winnerId.value) throw Error('请先选择本局胜方'); await action('recordFrame', { matchId: form.dataset.match, winnerId: form.elements.winnerId.value, loserScore: form.elements.loserScore.value }); closeModal(); }
